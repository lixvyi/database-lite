...
class Catalog:
    ...
    def create_table(self, schema):
        ...
        if key in self.tables:
            raise SemanticError(f"table '{schema.name}' already exists")
    ...
    def table(self, name, location=None):
        ...
        if not t:
            raise SemanticError(f"table '{name}' does not exist", location)
        return t
    ...
    def column(self, table, name, location=None):
        ...
        if not col:
            raise SemanticError(f"column '{name}' does not exist in table '{table.name}'", location)
        return col
...
class PagedCatalog:
    ...
    SYSTEM_SCHEMA = TableSchema('pg_catalog', [ColumnSchema('table_name', 'VARCHAR', 128), ColumnSchema('column_name', 'VARCHAR', 128), ColumnSchema('data_type', 'VARCHAR', 16), ColumnSchema('length', 'INT'), ColumnSchema('column_order', 'INT'), ColumnSchema('first_page', 'INT')])

    def __init__(self, page_store):
        """初始化对象状态和依赖。"""
        self.store = page_store
        self.page_id = self._ensure_page()
        self.tables = {}
        self.load()
    ...
    def _ensure_page(self):
        ...
        if not allocated:
            ...
            if page_id != 0:
                raise RuntimeError('catalog bootstrap page must be page 0')
            return page_id
        if 0 not in allocated:
            raise RuntimeError('reserved catalog page 0 is missing')
        return 0
    ...
    def load(self):
        ...
        if raw[:4] == self.LEGACY_MAGIC:
            _, size = self.LEGACY_HEADER.unpack_from(raw)
            data = json.loads(raw[self.LEGACY_HEADER.size:self.LEGACY_HEADER.size + size].decode('utf-8'))
    ...
    def save(self):
        ...
        for row in self.rows():
            ...
            if slot is None:
                ...
                pages[-1].next_page = page_id
                pages.append(SlottedPage(page_id))
                if pages[-1].insert(payload) is None:
                    raise SemanticError('one system catalog row is larger than one page')
        for page in pages:
            self.store.write_page(page.page_id, bytes(page.data))
        for page_id in old_pages[len(pages):]:
            self.store.release_page(page_id)

    def create_table(self, schema):
        """登记并持久化新表结构。"""
        key = schema.name.lower()
        if key in self.tables or key == 'pg_catalog':
            raise SemanticError(f"table '{schema.name}' already exists or is reserved")
        self.tables[key] = schema
        self.save()

    def exists(self, name):
        """判断指定对象是否存在。"""
        return name.lower() == 'pg_catalog' or name.lower() in self.tables

    def table(self, name, location=None):
        """查找并返回表结构。"""
        if name.lower() == 'pg_catalog':
            return self.SYSTEM_SCHEMA
    ...
    def rows(self):
        """生成当前节点对应的记录集合。"""
        return [{'table_name': t.name, 'column_name': c.name, 'data_type': c.data_type, 'length': c.length, 'column_order': i, 'first_page': t.first_page} for t in self.tables.values() for i, c in enumerate(t.columns)]
    ...
    def _catalog_pages(self):
        ...
        while page_id != NO_PAGE:
            if page_id in pages:
                raise RuntimeError('system catalog page chain contains a cycle')
    ...
    def _stored_rows(self):
        ...
        for page_id in self._catalog_pages():
            ...
            for _, payload in page.records():
                yield RecordCodec.decode(self.SYSTEM_SCHEMA, payload)
