...
>
> 对应版本：GitHub `lixvyi/database-lite`，主分支 `main`。
>
...
| OS 仿真台 | 独立 `os_sim/` | 展示页、缓存、脏页、WAL、Checkpoint 和任务队列 |
...
├─ benchmarks/os_benchmark.py    # 缓存、并发效果基准
...
`--inspect` 输出 Token、AST、优化前计划、优化后计划，不执行写入。
...
5. “运行 1000 查询”：8 个 worker 处理任务，等待队列最大容量 64，不创建 1000 个线程。
...
测试覆盖 25 项行为。基准重点看稳定指标而非某次毫秒数：

- 热读 1000 次新增磁盘读取为 0；
- 命中率约 99.9%；
- 8 worker 对 1000 个模拟 I/O 任务相对串行约 8 倍加速；
- 最大排队深度不超过 64。
...
子类只修改 `stage`：Lexical、Syntax、Semantic、Storage、Permission。好处是 API 只需捕获 `MiniDBError`，同时用户仍能看到错误发生阶段。
...
语句节点：CreateTableStmt、InsertStmt、SelectStmt、DeleteStmt、UpdateStmt、ExplainStmt。AST 只描述“用户写了什么”，不放磁盘页、锁或缓存对象。
...
- CREATE：检查重复列；真正的重复表检查由 PagedCatalog.create_table 在执行时完成；
- EXPLAIN：分析内部语句；
- 其他语句：先绑定表；
- pg_catalog：只有 SELECT 合法；
- INSERT：列数=值数、目标列不重复、列存在、值类型匹配；
- SELECT：输出列和排序列存在、WHERE 是 BOOL；
- DELETE：WHERE 是 BOOL；
- UPDATE：目标列不重复、目标列存在、赋值表达式类型匹配、WHERE 是 BOOL。
...
`expr_text()` 把表达式 AST 恢复成可读字符串，服务于 EXPLAIN，不用于执行。
...
3. 创建页 0 `PagedCatalog`；
4. 创建 OSBufferAdapter；
5. 创建 SemanticAnalyzer、PlanBuilder、Optimizer、Executor；
6. 创建 Authorizer。

`compile(sql)`：Lexer → Parser → Semantic → PlanBuilder → Optimizer，返回可观察的中间结果，不执行。

`execute(sql,user)`：按多语句顺序逐条分析和执行。为什么不能先分析完所有语句？因为 `CREATE TABLE; INSERT...;` 中第二条的语义环境依赖第一条已执行后的 Catalog。

权限步骤：确定动作类型和表名；admin 当前直接放行，其他用户查 Policy；SELECT 同时检查投影列与排序列，UPDATE 检查赋值目标列；行过滤器传入 Executor。
...
- `PagedCatalog`：当前主路径，元数据通过 StorageService 保存在页 0。

PagedCatalog 页格式：

```text
0..3    MAGIC = CAT1
4..7    JSON 字节长度 uint32 little-endian
8..N    UTF-8 JSON：表、列、类型、first_page
N..4095 零填充
```

`_ensure_page()`：空库首次分配必须拿到 page 0；已有库若 page 0 丢失则拒绝启动，避免悄悄创建错误目录。

`load()`：无 CAT1 时初始化空目录并保存；存在时读取长度、解码 JSON、重建 dataclass。

`save()`：序列化元数据，检查不能超过一页，补齐到 4096 字节，再走 `store.write_page()`。因此 Catalog 也有 WAL、缓存和恢复。

`SYSTEM_SCHEMA` 定义可查询视图 `pg_catalog`；`rows()` 将内部每列元数据展开成行。它逻辑上像特殊表，物理上是紧凑目录页，不是普通 SlottedPage。

### 7.2 `minidb/auth.py`

Policy 包含：user、table、actions、可选 columns、可选 row_filter。

四层含义：

- 业务/动作：SELECT、INSERT、UPDATE、DELETE；
- 表：策略只作用于某表；
- 列：允许访问的字段集合；
- 行：`row_filter(row)` 决定当前用户能看到或修改哪些行。

`grant()` 追加策略；`policy()` 找匹配项，无权限就抛 PermissionDenied；`check_columns()` 做集合包含判断。

测试示例让 alice 只能 SELECT staff 的 name/dept，并只能看 `dept=CS` 的行。当前策略保存在内存，没有用户表、密码哈希和持久化角色管理，这是明确边界。

### 7.3 `minidb/execution.py`：表达式
...
- SeqScan：普通表走 TableHeap；pg_catalog 走 Catalog.rows；行权限在这里最早过滤；
- Filter：evaluate(predicate,row) 为真才 yield；
- Project：构造只含指定列的新字典；
- Sort：先物化；从最后一个排序键向前做稳定排序，从而得到正确多键顺序；NULL 固定放末尾。
...
- INT：`struct.pack('<q')`，8 字节小端有符号整数；
- VARCHAR：`uint32 byte_length + UTF-8 bytes`。
...
- QueueOverloaded：有界任务队列已满且调用方选择不阻塞。
...
2. dirty ratio 达阈值；
3. 后台线程周期调用；
4. checkpoint/正常 close。

`checkpoint()` 必须先 flush_all，再写 control.checkpoint_lsn，顺序不能反。

### 9.7 `os_sim/scheduler.py`

Task 保存序号、提交时间、函数、参数和 Future。

QueryScheduler 初始化固定数量 daemon worker 和固定容量 Queue。

`submit()`：生成序号/Future/Task；Queue 满时阻塞，或 block=False 时抛 QueueOverloaded；记录 submitted 和 max_depth。

`_worker()`：循环 get；None 是停止哨兵；记录等待时间；Future 被取消就跳过；执行函数并设置结果或异常；最后 task_done。

`stats()`：workers、capacity、当前/最大深度、成功失败拒绝、平均等待、p95 等待。

`close()`：先 drain 已接收任务，再每个 worker 放一个 None，最后 join。它解决的是资源边界和背压，不提供 SQL 优先级、超时取消或分布式调度。

### 9.8 `os_sim_cli.py` 和 benchmark

CLI 的 `fixed_payload()` 把 UTF-8 文本零填充到 4096 字节。argparse 子命令映射 StorageService 方法；finally 始终 close。

benchmark 用 `perf_counter()`：测一次冷读、1000 次热读；再对比 1000 个 1ms 串行任务与 8 worker 有界并发。`tempfile` 保证不污染正式数据库。

### 9.9 `os_sim/__init__.py`

包入口只公开 StorageService 和 QueryScheduler。上层无需导入 PageFile、PageCache、WAL 再自行拼装，防止绕过统一恢复与刷页顺序；需要做白盒测试时才直接进入具体子模块。
...
- OS action：分配、写页、checkpoint、1000 查询。
...
`osAction()`：write 时选择一个已分配页，没有就先分配；workload 完成后 toast 最大队列深度；所有操作后重新拉取真实状态。
...
`minidb_cli.py` 用 argparse 接收 `--data`、`--sql` 和 `--inspect`。有 `--sql` 时执行一次并输出 JSON；没有时进入 REPL，把输入累积到分号后执行；`.quit` 退出。只捕获 MiniDBError，因此编译错误按课程阶段显示，而真正的程序错误不会被伪装成 SQL 错误。
...
- `docs/*.md`：三个阶段的验收矩阵、设计取舍、测试报告、双人报告和答辩材料，不参与运行；
...
- 1000 查询全部完成且 max_depth≤64。

### 11.3 `tests/test_api.py`

在随机空闲端口启动真实 ThreadingHTTPServer，SQLite 指向临时文件。验证 dashboard、新增图书、借还库存，以及 MiniDB API 能输出 Sort、Catalog 和已分配页。

### 11.4 `tests/test_fuzz.py`
...
| 数据持久化 | Catalog 和行都走页服务 | 300 行关闭/重开测试 |
| 并发控制 | 表级 RWLock | 多读共享、写独占结构 |
| 1000 查询 | 8 worker + Queue(64) | completed=1000、max_depth≤64 |
| 权限 | 动作+表+列+行过滤 | alice 权限端到端测试 |
...
两周实训中表锁最容易保证正确、避免死锁图和锁升级复杂度。代价是写操作并发度有限。

### 有界线程池而不是每请求一线程

1000 查询不应创建 1000 个工作线程；固定 worker 控制 CPU/文件竞争，容量 64 提供背压。代价是高峰请求需要等待或重试。

### 页 0 Catalog 而不是独立 catalog.json

若元数据在独立 JSON，而业务行在页文件，“所有数据和元数据通过页存储持久化”的要求没有真正完成。页 0 提供固定启动入口，并自动获得缓存、WAL、CRC 和恢复能力。
...
讲解 catalog、execution、record、page、os_adapter、PageFile、Cache、WAL、StorageService、Scheduler、server 接口。
...
6. 2 分钟：Cache、WAL、Checkpoint、1000 查询；
7. 1 分钟：25 项测试、边界和双人贡献。
...
QueryScheduler 固定 8 worker、Queue 容量 64；任务满时提交方背压，不无限创建线程和占用内存。
...
本文的核心记忆句：SQL 文本由编译器变成 Plan，Executor 把 Plan 变成 Row/RID 操作，TableHeap 把 Row/RID 变成 SlottedPage 操作，StorageService 再把完整 4KB 页通过 Cache 和 WAL 安全地映射到磁盘。
