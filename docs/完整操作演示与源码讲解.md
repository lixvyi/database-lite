...
只看编译过程。为了让命令能在全新的数据目录中独立运行，把建表和查询放在同一次 `--inspect` 中：

```powershell
py -3 minidb_cli.py --data inspect_data --inspect --sql "CREATE TABLE demo(name VARCHAR(20),score INT); SELECT name FROM demo WHERE score > 10 + 8;"
```

`--inspect` 输出 Token、AST、语义检查结果、优化前计划和优化后计划，不执行写入，但仍会严格检查表名和列名。若只检查 `SELECT name FROM demo...`，必须先在同一个 `--data` 目录中真正创建 `demo` 表，否则正确结果就是 `table 'demo' does not exist`。
