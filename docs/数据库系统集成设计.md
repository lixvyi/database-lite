...
| 系统目录 | 保留页 0，`CAT1 + JSON长度 + JSON + 填充` |
...
3. 每次写入是完整页 after-image，WAL 重放和 CRC 校验都比局部补丁更直观，适合两周实训答辩。
...
- CreateTable：写入 `PagedCatalog`，目录页 0 变脏并经 WAL/页服务落盘。
...
ORDER BY 在 Filter 后、Project 前插入 Sort，这样即使排序列没有出现在最终投影中也能正确排序。多列排序从最后一个键向前做稳定排序；NULL 固定放最后。

## 同步、崩溃与恢复

`write_page` 的顺序是：完整页追加 WAL → `fsync(WAL)` → 内存 frame 标脏。脏页达到比例阈值、被淘汰、后台周期到达或显式 checkpoint 时才刷回。刷页前检查 `page_lsn <= durable_lsn`，否则抛出 `WriteBarrierError`。
