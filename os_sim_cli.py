...
def main():
    ...
    release.add_argument('page_id', type=int)
    read = sub.add_parser('read')
    read.add_argument('page_id', type=int)
    ...
    try:
        if args.command == 'status':
            ...
            store.release_page(args.page_id)
            result = {'released': args.page_id}
        elif args.command == 'read':
            result = {'page_id': args.page_id, 'text': store.read_page(args.page_id).rstrip(b'\x00').decode('utf-8')}
        elif args.command == 'write':
            result = {'page_id': args.page_id, 'lsn': store.write_page(args.page_id, fixed_payload(args.text))}
        else:
