...
class StorageService:
    ...
    def __init__(self, directory, cache_pages=16, policy='LRU', dirty_ratio=0.6):
        ...
        self.cache = PageCache(self.file, self.wal, cache_pages, policy)
    ...
    def release_page(self, page_id):
        ...
        if frame and frame.pin_count:
            raise RuntimeError('cannot release a pinned page')
    ...
    def stats(self):
        """返回缓存和存储运行统计。"""
        return {**self.cache.stats(), 'wal_durable_lsn': self.wal.durable_lsn, 'checkpoint_lsn': self.file.checkpoint_lsn, 'recovered_pages': self.recovered_pages, 'allocated_pages': sum(self.file.control['allocated']), 'total_pages': self.file.control['page_count']}
