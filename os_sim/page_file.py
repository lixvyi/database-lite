...
class PageFile:
    ...
    def __init__(self, directory: str | Path):
        ...
        self.lock = RLock()
        self.reads = self.writes = self.allocations = self.releases = 0
        if not self.data_path.exists():
            self.data_path.write_bytes(b'')
        self._control_needs_save = False
        self.control = self._load_control()
        if self._control_needs_save:
            self._save_control()

    def _default_control(self):
        """完成default control相关处理。"""
    ...
    def _load_control(self):
        ...
        if control.get('version') == 1:
            ...
            control['version'] = 2
            control['page_generations'] = [1 if used else 0 for used in control['allocated']]
        ...
        if control['page_count'] * PAGE_SIZE > self.data_path.stat().st_size:
            raise CorruptPage('control file points beyond tablespace')
        return control
    ...
    def _grow_extent(self):
        ...
        self.control['checksums'].extend([0] * EXTENT_PAGES)
        self._save_control()
        return start
    ...
    def allocate_page(self):
        ...
        with self.lock:
            try:
                page_id = self.control['allocated'].index(False)
            except ValueError:
    ...
    def release_page(self, page_id):
        ...
        with self.lock:
            ...
            self.control['allocated'][page_id] = False
    ...
    def _validate(self, page_id):
        """完成validate相关处理。"""
        if page_id < 0 or page_id >= self.control['page_count'] or (not self.control['allocated'][page_id]):
            raise InvalidPage(f'page {page_id} is not allocated')
    ...
    def read_page(self, page_id):
        ...
        with self.lock:
            ...
            if len(payload) != PAGE_SIZE or (expected and zlib.crc32(payload) != expected):
                raise CorruptPage(f'checksum mismatch on page {page_id}')
    ...
    def set_checkpoint(self, lsn):
        """完成set checkpoint相关处理。"""
        self.control['checkpoint_lsn'] = lsn
        self._save_control()

    @property
    def checkpoint_lsn(self):
        """完成checkpoint lsn相关处理。"""
        return self.control['checkpoint_lsn']
