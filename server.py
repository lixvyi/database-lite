        ...
        try:
            ...
            with connect() as db:
                if path == '/api/books':
                    ...
                    cur = db.execute('INSERT INTO books(isbn,title,author,category,publisher,total_copies,available_copies,shelf_code) VALUES(?,?,?,?,?,?,?,?)', (data['isbn'], data['title'], data['author'], data.get('category', '其他'), data.get('publisher', ''), total, total, data.get('shelf_code', '')))
                    return self.send_json({'id': cur.lastrowid, 'message': '图书已入库'}, 201)
                if path == '/api/readers':
                    required(data, 'card_no', 'name')
                    cur = db.execute('INSERT INTO readers(card_no,name,phone,department,status) VALUES(?,?,?,?,?)', (data['card_no'], data['name'], data.get('phone', ''), data.get('department', ''), data.get('status', '正常')))
                ...
                if path == '/api/loans':
                    ...
                    if not book or book['available_copies'] < 1:
                        raise ApiError(409, '该图书当前无可借副本')
                    if not reader or reader['status'] != '正常':
                        raise ApiError(409, '读者不存在或已停用')
                    today = date.today()
                    due = today + timedelta(days=int(data.get('days', 14)))
                    ...
                    cur = db.execute("INSERT INTO loans(loan_no,book_id,reader_id,borrowed_at,due_at,status) VALUES(?,?,?,?,?,'借阅中')", (loan_no, book['id'], reader['id'], today.isoformat(), due.isoformat()))
                ...
                if path == '/api/os/action':
                    ...
                    if action == 'allocate':
                        result = {'page_id': OS_STORE.allocate_page()}
                    elif action == 'write':
                        pid = int(data['page_id'])
                        raw = str(data.get('text', '页面数据')).encode('utf-8')
                        if len(raw) > PAYLOAD_SIZE:
                            raise ApiError(400, '内容超过一页容量')
            ...
            self.send_json({'error': '编号已存在或数据不符合约束', 'detail': str(exc)}, 409)
        except Exception as exc:
            self.send_json({'error': f'服务器错误：{exc}'}, 500)
        ...
        try:
            ...
            if len(parts) == 4 and parts[:2] == ['api', 'loans'] and (parts[3] == 'return'):
                ...
                with connect() as db:
                    ...
                    if not loan:
                        raise ApiError(404, '借阅记录不存在')
                    if loan['status'] == '已归还':
                        raise ApiError(409, '该图书已经归还')
...
def required(data, *keys):
    ...
    if missing:
        raise ApiError(400, '请填写必填字段：' + '、'.join(missing))
